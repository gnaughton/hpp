...
class ShowmeProcessor
  ...
  def loadFiles (lang, settings_file_root)
    ...
    @LIST_TEMPLATE = File.read($CONFIG_FILES_ROOT + "files/user/showmes/list_link_template_" + lang + ".txt")
    @CONTEXT_TEMPLATE = File.read($CONFIG_FILES_ROOT + "files/user/showmes/context_link_template_" + lang + ".txt")
  ...
  def copyContextualIcon(webhelp_path)
    ...
    FileUtils.cp $CONFIG_FILES_ROOT + "files/user/showmes/i_help_video.png", webhelp_path + "/i_help_video.png"
